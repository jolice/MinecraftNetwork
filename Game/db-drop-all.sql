...
alter table kit drop constraint if exists fk_kit_kit_permissions_id;

alter table kit_inventory_item_entity drop constraint if exists fk_kit_inventory_item_entity_kit;

alter table kit_inventory_item_entity drop constraint if exists fk_kit_inventory_item_entity_inventory_item_entity;

alter table kit_description drop constraint if exists fk_kit_description_kit_id;
drop index if exists ix_kit_description_kit_id;

alter table kit_permissions drop constraint if exists fk_kit_permissions_required_rank_id;
drop index if exists ix_kit_permissions_required_rank_id;
...
drop sequence if exists purchasable_sequence;

drop table if exists kit_inventory_item_entity;

drop table if exists kit_description;

drop table if exists kit_permissions;
